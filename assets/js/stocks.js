...
function extractAndSetTargetPrices() {
    ...
    // SIMPLIFIED APPROACH: Just use fixed percentage of current price
    let buyPrice = 0;
    let sellPrice = 0;
    
    if (currentPrice > 0) {
        // Set buy price to 85% of current price (15% discount)
        buyPrice = Math.round(currentPrice * 0.85 * 100) / 100;
        
        // Set sell price to 115% of current price (15% gain)
        sellPrice = Math.round(currentPrice * 1.15 * 100) / 100;
    }
